...
if ($status != 0) then
   echo "ERROR: Cannot find container directory : $CONTR_URL"
   exit 1
endif

if {(test -e $PROJT_PATH/$CONTAINER_PATH/.svn)} then
   svn update --quiet --force $PROJT_PATH/$CONTAINER_PATH --set-depth infinity
else
   svn checkout --force $CONTR_URL $PROJT_PATH/$CONTAINER_PATH --depth infinity
endif
